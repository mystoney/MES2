   ...
   public class CopyFile
    {
            ...
            try
            {
                ...
                if (sourceDirPath_state == false)
                {
                    ...
                    if (sourceDirPath_key != false)
                    {
                        ...
                        //连接共享文件夹
                        status = connectState(@copyfile.string_Route(sourceDirPath), source_username, source_userpwd);
                ...
                foreach (string file in files_list)
                {
                    ...
                    string pFilePath = saveDirPath + "\\" + file.Trim().Replace("http:", "").Replace(":", "").Substring(2);
                    CreatePath(pFilePath);
                    ...
                    if (BackupDirectory != "")
                    {
                        ...
                        //string BFilePath = BackupDirectory + "\\" + Path.GetDirectoryName(file.Trim().Replace("http", "").Replace(":","")).Substring(2);
                        string BFilePath = BackupDirectory + "\\" + file.Trim().Replace("http", "").Replace(":", "").Substring(2);
                        CreatePath(BFilePath);
        ...
        public static bool connectState(string path, string userName, string passWord)
        {
            ...
            //}
            CopyFile copyfile = new CopyFile();
            Decrypt_out decrypt_out = new Decrypt_out();
            ...
            try
            {
                ...
                string dosLine = @"net use " + path + " /del /y";
                proc.StandardInput.WriteLine(dosLine);
                proc.StandardInput.WriteLine("exit");

                //string LinkDuplication = decrypt_out.Decrypt_out_Str("LinkDuplication");

                // dosLine = @"net use " + LinkDuplication + "  /del /y";
                //proc.StandardInput.WriteLine(dosLine);
                //proc.StandardInput.WriteLine("exit");
        ...
        #region 判断路径是否存在不存在就建立路径

        /// <summary>
        /// 判断路径是否存在不存在就建立路径
        /// </summary>
        /// <param name="Path_all"></param>
        public void CreatePath(string Path_all)
        {
            string path = Path.GetDirectoryName(Path_all.Trim());
        ...
        #region 判断远程文件夹是否可以访问

        /// <summary>  
        /// 判断远程文件夹是否可以访问 
        /// </summary>  
        /// <param name="path">搜索的目录</param>  
        public static bool GetDir(string path)
        {
